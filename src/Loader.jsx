...
export default function Loader() {
  ...
  const stairsVariants = {
    ...
    exit: (i) => ({
      ...
      transition: {
        duration: 0.7,
        ease: [0.76, 0, 0.24, 1], // Custom bezier for premium feeling
        delay: i * 0.08,
      }
    })
  };
  ...
  return (
    <motion.div
      className="fixed top-0 left-0 w-full h-[100dvh] flex flex-col justify-center items-center z-[9999] overflow-hidden bg-transparent"
      initial={{ opacity: 1 }}
      exit={{ opacity: 1 }}
      style={{ color: loaderText }}
    >
      {/* STAIRS PANELS (BACKGROUND) */}
      ...
      >
        ...
        >
          ...
          <AnimatePresence mode="wait">
            {bubbleText && (
              <motion.div 
                key={bubbleText}
                className="absolute bottom-[95%] left-1/2 -translate-x-1/2 md:bottom-[90%] md:left-[55%] md:translate-x-0 mb-3 font-space text-[0.9rem] px-4 py-2.5 rounded-xl shadow-md whitespace-normal w-max max-w-[240px] sm:max-w-[320px] text-left z-10"
                style={{ backgroundColor: loaderText, color: loaderBg }}
              ...
              >
                ...
                <div 
                  className="absolute top-[100%] left-1/2 -translate-x-1/2 md:left-[12px] md:translate-x-0 w-0 h-0 border-t-[7px] border-x-[7px] border-x-transparent"
                  style={{ borderTopColor: loaderText }}
                ></div>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          <svg viewBox="0 0 40 40" className="w-24 h-24" style={{ overflow: "visible", color: loaderText }}>
            {/* Body */}
            <rect x="12" y="18" width="16" height="14" rx="4" fill="currentColor" />
            {/* Head */}
            <circle cx="20" cy="11" r="8" fill="currentColor" />
            {/* Visor */}
            <rect x="14" y="8" width="12" height="6" rx="2" fill={loaderBg} className="avatar-visor" />
            {/* Eyes */}
        ...
        {/* Progress Bar */}
        <div 
          className="w-[260px] h-[3px] mt-3 rounded overflow-hidden relative shadow-[0_0_10px_rgba(0,0,0,0.5)]"
          style={{ backgroundColor: progressTrackBg }}
        >
          <motion.div
            className="h-full w-0 bg-[length:200%] animate-[roleGradient_2s_linear_infinite]"
            style={{ backgroundImage: `linear-gradient(90deg, ${loaderText}, ${loaderAccent}, ${loaderText})` }}
            initial={{ width: 0 }}
            animate={{ width: "100%" }}
            transition={{
              duration: 7.8,
